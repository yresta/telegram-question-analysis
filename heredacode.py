...
def generate_representative(questions: List[str]) -> str:
    ...
    cleaned_questions = preprocess_questions(questions)

    embeddings = model.encode(cleaned_questions)

    num_clusters = min(len(cleaned_questions), 5) 
    ...
    for center in cluster_centers:
        ...
        representative_questions.append(cleaned_questions[closest_idx])

    representative_sentence = max(set(representative_questions), key=representative_questions.count)
